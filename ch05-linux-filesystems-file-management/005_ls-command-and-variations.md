# The **ls** command and some of its options: </br>  
### **ls** - list the contents of a directory </br>  
### **ls -l** - list the contents of a directory in the long listing format, providing more information. </br>  
### **ls -la** - the **-a** option reveals the hidden directories and files in a directory. </br>  
### **ls -ld** - the **-d** option enables ls to list the directory and not its contents. </br>  
The image below shows the usage of the above commands </br>  
<img width="824" height="935" alt="Screenshot From 2026-06-10 22-42-09" src="https://github.com/user-attachments/assets/ec26460a-ad99-4e65-bb7f-a11acd37a615" /> </br>  


### **ls -li** - the **-i** options displays the **inode** number for the contents of a directory. </br>  
### **ls -lt** - the **-t** option sorts contents of a directory by time, with the newest first. </br>  
### **ls -ltr** - the **-r (lowercase)** option sorts contents in the reverse order. So if **-t** sorts by newest first, **-tr** displays newest at the last. </br>  
### **ls -lh** - the **-h** option displays the size of the contents in human-readable format: that is K(Kilobytes), M(Megabytes) and so on. </br>  
The image below shows the usage of the above commands </br>  
<img width="599" height="992" alt="Screenshot From 2026-06-10 22-44-05" src="https://github.com/user-attachments/assets/f737edff-811c-451a-990a-e41aee125e2d" /> </br>  


### **ls -lR** - the **-R** (uppercase) option displays the contents of the directories in a recursive manner. </br>  
<img width="489" height="537" alt="Screenshot From 2026-06-10 22-47-51" src="https://github.com/user-attachments/assets/4c4faed5-95c7-4946-9703-044b91d9ee27" /> </br>  
